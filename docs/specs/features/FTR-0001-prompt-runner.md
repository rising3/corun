...
`log-dir` | --log-dir `<dir>` | 省略時は copilot-cli デフォルト。`--log-dir` コマンドラインフラグが同時指定された場合はコマンドラインフラグを優先し、このフィールドの値を無視する
`log-level` | --log-level `<level>` | 省略時は copilot-cli デフォルト。`--log-level` コマンドラインフラグが同時指定された場合はコマンドラインフラグを優先し、このフィールドの値を無視する
...
- `text` フィールドが空文字または未設定のエントリは実行をスキップし、警告メッセージを stderr に出力して後続プロンプトの実行を継続する。すべてのエントリが `text` 未設定によりスキップされた場合は `prompts: []` と同等に扱い、終了コード 0 で正常終了する
- `retries.backoff` に `"linear"` 以外の値が指定された場合は終了コード 1 (EXIT_ERROR) で即時終了し、未サポートの値である旨を stderr に出力する
- `on_failure.action` に `"abort"` 以外の未知の値が指定された場合は終了コード 1 (EXIT_ERROR) で即時終了し、未サポートの値である旨を stderr に出力する（`retries.backoff` における未サポート値と同一ポリシー）
...
4. **プロンプト定義をパースする**
   ...
   - `on_failure.action` に `"abort"` 以外の未知の値が設定されているエントリが存在する場合は終了コード 1 (EXIT_ERROR) で即時終了し、未サポートの値である旨を stderr に出力する
...
5. **プロンプトを順番に実行する** (`prompts[]` の定義順)
   ...
   2. **copilot CLI を実行する**
      ...
      - `timeout_seconds` が設定されている場合は、タイムアウト経過で copilot CLI プロセスに SIGTERM を送信し、直後（0 秒）に SIGKILL で確実に終了させ、非ゼロ終了と同等の「失敗」として扱う（次の「失敗時の処理」ステップへ進む）。タイムアウトカウントは **1 試行ごとに独立して適用**され、再試行時にはリセットされる（再試行を含む合計時間ではない）
...
1 | copilot CLI が見つからない / YAML パース失敗 / ファイルが存在しない / copilot CLI の実行エラー / タイムアウト後に on_failure: abort / on_failure 未設定で継続したが 1 つ以上のプロンプトが失敗した / `retries.backoff` に未サポートの値が指定された / `on_failure.action` に未サポートの値が指定された
