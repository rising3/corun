...
6. **終了する**
   ...
   - SIGINT を受信した場合は終了コード 2 (EXIT_CANCEL) で即時終了する。`retries.delay_seconds` による待機中であっても即時検知して終了する（待機完了を待たない）。実行中の copilot CLI 子プロセスがある場合は SIGTERM → SIGKILL（0 秒猶予）を送信して確実に終了させる（子プロセスの残留・ゾンビ化を防ぐ）
...
1 | copilot CLI が見つからない / YAML パース失敗 / ファイルが存在しない / copilot CLI の実行エラー / タイムアウト後に on_failure: abort / on_failure 未設定で継続したが 1 つ以上のプロンプトが失敗した / `retries.backoff` に未サポートの値が指定された / `on_failure.action` に未サポートの値が指定された / `retries.count`・`retries.delay_seconds`・`timeout_seconds` に負値が指定された
