...
- `text` フィールドが空文字または未設定のエントリは実行をスキップし、`warning: <説明>` 形式で警告を stderr に出力して後続プロンプトの実行を継続する。すべてのエントリが `text` 未設定によりスキップされた場合は `prompts: []` と同等に扱い、終了コード 0 で正常終了する
...
- `env` フィールドに設定された環境変数は、corun の実行環境変数に追加・上書きしてマージしたうえで copilot CLI の実行環境に渡す（既存の環境変数は保持され、`env` で指定したキーのみが追加または上書きされる）。サニタイズは一切行わない。API キー・パスワード等の機密情報を `env` に設定しないことはユーザーの責任とする
...
4. **プロンプト定義をパースする**
   ...
   - 各 `prompts[]` エントリの `text` フィールドが空文字または未設定の場合は当該エントリをスキップし `warning: <説明>` 形式で警告を stderr に出力して後続を継続する
...
5. **プロンプトを順番に実行する** (`prompts[]` の定義順)
   1. **copilot CLI フラグを組み立てる**
      ...
      - プロンプト個別の `env` が設定されている場合は、corun の実行環境変数に追加・上書きしてマージしたうえで copilot CLI に渡す（既存環境変数を保持し、`env` 指定キーのみを追加または上書きする）
   2. **copilot CLI を実行する**
