...
5. **プロンプトを順番に実行する** (`prompts[]` の定義順)
   ...
   2. **copilot CLI を実行する**
      ...
      - `timeout_seconds` が設定されている場合は、タイムアウト経過で強制終了し copilot CLI の非ゼロ終了と同等の「失敗」として扱う（次の「失敗時の処理」ステップへ進む）。タイムアウトカウントは **1 試行ごとに独立して適用**され、再試行時にはリセットされる（再試行を含む合計時間ではない）
